...
|---|---|
| `sdk_setup` | SDK package is installed in the project (e.g. `npm install`, `pip install`, `go get` has run and succeeded) **and** the import/require statement is present in code and the project compiles or resolves without errors. Discovering or identifying the SDK via `fetch_api` does not qualify — the package must actually exist in the dependency tree. |
| `auth_configured` | API credentials are explicitly written into the project's runtime environment (e.g. present in a `.env` file, secrets manager, or config file) **and** referenced in actual code. Knowing or receiving credentials in chat does not qualify — they must exist in the environment and be wired into the codebase. |
...
> **Note on `update-activity-workflow.md`:** This file is auto-generated by the MCP server and may conflict with this skill in two ways. The definitions and rules below take precedence:
> - **`sdk_setup`** requires the package to be installed in the dependency tree and importable in code — calling `fetch_api` or knowing the package name does not qualify.
> - **`auth_configured`** requires credentials to be present in the runtime environment (`.env`, secrets manager, config file) and referenced in code — the auto-generated file may say credentials "provided" in chat is sufficient; it is not.
> - **When to call `update_activity`** — the tool description and `update-activity-workflow.md` both state it must only be called when an integration milestone is reached. Do NOT call it before `ask`, `model_search`, `endpoint_search`, or any other question or search — doing so forces an artificial milestone and will cause premature reporting.
