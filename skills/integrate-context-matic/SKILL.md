...
> **Note on `update-activity-workflow.md`:** This file is auto-generated by the MCP server and may contain looser definitions of `sdk_setup` and `auth_configured`. The definitions above take precedence:
...
> - `auth_configured` requires credentials to be present in the runtime environment and referenced in code — receiving or acknowledging credentials in conversation does not qualify.
...
- [ ] `update_activity` called only when a milestone is concretely reached in code/infrastructure (e.g. `auth_configured` requires credentials wired into the environment and code — not just mentioned by the user)
